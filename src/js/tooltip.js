...
//         |   ____|
//          \/
//          [orient = top, point = right]
//          _______
//         |       |
//        < _______|
//          [orient = right, point = bottom]
//   point  : Треугольник Относительно линии прикрепления тела Подсказки к треугольнику.
//   orient : Ориентация точки Относительно тела подсказки
...
CeTooltip.prototype = {
	...
	refresh: function () {
		...
		var $this = this.element, tooltip = this.getElement(), tooltip_params = {height : tooltip.height(), width : tooltip.width()}, offset = $this.offset(), eo = {
	...
	_create         : function () {
		...
		$("body").append(
			'<div id="' + this.getIdString() + '" ' + A_SID + '="' + this.id + '" class="' + classes + '" style="' + this.getOption("style") + '">' +
				'<div class="' + getElmClass("block") + '">' +
					'<span class="ceb_tr1 ceb_tr"></span>' +
					'<span class="ceb_tr2 ceb_tr"></span>' +
					'<div class="' + getElmClass("message") + '">' + this.getOption("message") + '</div>' +
					'<div class="' + getElmClass("close") + '">' + this.getOption("closeText") + '</div>' +
				'</div>' +
			'</div>'
		);
